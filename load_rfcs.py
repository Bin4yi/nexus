...
What it does:
    ...
    2. Creates Specification nodes in SQLite
    ...
    6. Writes IMPLEMENTS_SPEC edges into SQLite
"""
import logging
import sys
...
def main():
    ...
    # ── Load Specification nodes + sections into SQLite ───────────────────────
    from graph.sqlite_loader import SQLiteLoader
    loader = SQLiteLoader()
    loader.open()

    loader.load_specification_nodes(specs)
    logger.info("Specification nodes loaded into SQLite")

    sections = parser.chunk_rfc_sections(specs)
    loader.load_specification_section_nodes(sections)
    ...
    if llm_edges:
        ...
        logger.info("Loaded %d IMPLEMENTS_SPEC edges into SQLite", len(llm_edges))
    else:
        logger.warning(
    ...
    logger.info("Done. Verify in SQLite:")
    logger.info("  SELECT * FROM rfc_specs LIMIT 25;")
    logger.info("  SELECT * FROM implements_spec ORDER BY rowid DESC LIMIT 20;")
